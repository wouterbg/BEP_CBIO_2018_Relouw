...
switch char(expname) % parameters change after intervention
    ...
    modVars.IM1influxProb = linspace(0.4,0.8,nI);
    ...
    case 'IMmobile'
    modVars.IM1pmig = linspace(0.3,0.6,nI);
    modVars.IM1rwalk = linspace(0.3,0.9,nI); 
    
    case 'IMfierce'
    modVars.IM1pkill = linspace(0.1,0.7,nI);
    modVars.IM1rwalk = linspace(0.3,0.9,nI); 
    ...
    case 'IMnum'
    modVars.IM1influxProb = linspace(0.1,0.9,nI);
    modVars.IM1inflRate = 1:5; 
    
    case 'PermIM'
    modVars.IM1influxProb = linspace(0.45,0.85,nI);  
    ...
    timeFirstRun = [50 50];
    addSteps = 300; 
    modVars.IM1influxProb = [0.60,0.62,0.74,0.75,0.80];
    ...
    timeFirstRun = [50 50];
    addSteps = 300; 
    modVars.IM1influxProb = [0.4,0.75];
    modVars.IM1pkill = [0.05,0.1,0.25];
...
%     modVars.IM1inflRate =   [8, 8, 8, 8];
%     %overrideBefore.IM1inflRate = 0;
%     
...
%     addSteps = 300; 
%     modVars.probSeedFibr = [0.06, 0.06];
%     modVars.IM1influxProb =  [0.4, 0.4, 0.4, 0.4];    
%     %overrideBefore.IM1influxProb = 0;
%     
...
% +++++++++++++++++
    case 'test_INTERVENTION_HI_IM1_HI_TUpblock'
    domainSize = [400 400]; % was 500 500
    timeFirstRun = [60 15]; % was 120 30
    addSteps = 240;  % was 360
    modVars.TUpblock = repmat(0.7,[1 5]);       %was repmat(0.06,[1 5]); 
    modVars.IM1inflRate =   repmat(8,[1 10]);   %was repmat(8,[1 10]);  
    overrideBefore.IM1inflRate = 0;


    case 'INTERVENTION_HI_IM1_HI_TUpblock'
    domainSize = [400 400]; % was 500 500
    ...
    case 'INTERVENTION_LO_IM1_HI_TUpblock'
    domainSize = [400 400]; % was 500 500
    timeFirstRun = [120 15]; % was 120 30
    addSteps = 240;  % was 360
    modVars.TUpblock = repmat(0.7,[1 5]);       %was repmat(0.06,[1 5]); 
    modVars.IM1inflRate =   repmat(2,[1 10]);   %was repmat(8,[1 10]);  
    %overrideBefore.IM1inflRate = 0;
    ...
    addSteps = 240;  % was 360 
    modVars.probSeedFibr =  repmat(0.06,[1 5]);   %was repmat(0.06,[1 5]); 
    modVars.IM1influxProb =  repmat(0.2,[1 10]);   %was repmat(0.4,[1 10]);  
    %overrideBefore.IM1influxProb = 0;
    ...
    modVars.IM1influxProb = 0.2:0.07:1; 
    ...
    modVars.IM1influxProb = 0.1:0.1:1;  
    clear overrideBefore
    ...
    timeFirstRun = [250 50];
    addSteps = 250; 
    modVars.IM1inflRate =  2:2:12;
    modVars.IM1influxProb = repmat(0.4,[1,11]);  
    clear overrideBefore
    overrideBefore.probSeedFibr = 0.06;    
    overrideBefore.IM1influxProb = 0.4; 
    overrideBefore.stromaPerm = 0; 
    overrideBefore.IM1pkill = 0.6; 
    
    case 'THERAPY_IMMUNE_STROMAPERM' % PREP DONE
    domainSize = [400 400];
    timeFirstRun = [300 50];
    addSteps = 300; 
    modVars.IM1inflRate =  3:1:12;
  ...
  otherwise % ------- ALL L1 L2 L3 L4 levels
   ...
   % overrideBefore.IMpkill = 0.1; % was 0.25
	...
	modVars.IM1inflRate = repmat(myInfl(1),repeatMe1);
    ...
    modVars.stromaPerm = repmat(myPerm(4),repeatMe2); 	
		...
		modVars.IM1inflRate = repmat(myInfl(2),repeatMe1);
		modVars.stromaPerm = repmat(myPerm(1),repeatMe2); 
		...
		modVars.stromaPerm = repmat(myPerm(4),repeatMe2); 	
			...
			modVars.IM1inflRate = repmat(myInfl(3),repeatMe1);
			modVars.stromaPerm = repmat(myPerm(1),repeatMe2); 
			...
			modVars.stromaPerm = repmat(myPerm(4),repeatMe2); 	
				...
				modVars.IM1inflRate = repmat(myInfl(4),repeatMe1);
				modVars.stromaPerm = repmat(myPerm(1),repeatMe2); 

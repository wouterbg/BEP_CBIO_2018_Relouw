...
function visualize_balls_2D_blank(mySystem)
    ...
    IM1colors = flipud(double(blugr(double(mySystem.params.IM1kmax)+3))); % color map for immune cells (1)
    IM2colors = flipud(double(purgr(double(mySystem.params.IM2kmax)+3))); % color map for immune cells (2)
    ...
    try % try to draw immune cells (1), skip if this is impossible
    % create immune cell coordinates
    yim = mod(mySystem.IM.IM1cells,mySystem.grid.N);
    xim = ceil(mySystem.IM.IM1cells/mySystem.grid.N);
    % retrieve immune cell colors
    myImc = IM1colors(mySystem.IM.IM1prop.Kcap+1,:);
    % plot immune cells
    ...
    try % try to draw immune cells (2), skip if this is impossible
    % create immune cell coordinates
    yim = mod(mySystem.IM.IM2cells,mySystem.grid.N);
    xim = ceil(mySystem.IM.IM2cells/mySystem.grid.N);
    % retrieve immune cell colors
    myImc = IM2colors(mySystem.IM.IM2prop.Kcap+1,:);
    % plot immune cells
    scatter(xim,yim,mySystem.params.mycellsize,myImc,'filled')
    catch
        disp('no immune cells could be plotted');
